...
function msr_publishing_render_fullscreen_nav_sublist( $type ) {
	...
	?>
	<div class="publishing-fullscreen-nav__subgroup">
		<p class="publishing-fullscreen-nav__eyebrow"><?php echo esc_html( $eyebrow ); ?></p>
		<?php if ( ! empty( $terms ) ) : ?>
		<ul class="publishing-fullscreen-nav__sublist">
			<?php foreach ( $terms as $term ) : ?>
				...
				$sub_class   = 'publishing-fullscreen-nav__sublink';
				if ( $sub_current ) {
					$sub_class .= ' is-current';
				}
				?>
				<li class="publishing-fullscreen-nav__subitem">
					<a
		...
		<?php endif; ?>
		<?php if ( $view_all ) : ?>
			<a class="publishing-fullscreen-nav__view-all" href="<?php echo esc_url( $view_all ); ?>"><?php echo esc_html( $view_label ); ?></a>
		<?php endif; ?>
	</div>
	<?php
}
...
function msr_publishing_render_nav_megamenu_panel( $type, $panel_id, $parent_url, $parent_label ) {
	...
	<div
		...
		class="publishing-megamenu"
		role="region"
		aria-label="<?php echo esc_attr( sprintf( /* translators: %s: nav section */ __( '%s menu', 'msrsandbox' ), $parent_label ) ); ?>"
		hidden
	>
		<p class="publishing-megamenu__eyebrow"><?php echo esc_html( $eyebrow ); ?></p>
		<ul class="publishing-megamenu__list">
			<?php foreach ( $terms as $term ) : ?>
				<li class="publishing-megamenu__item">
					<a class="publishing-megamenu__link" href="<?php echo esc_url( get_term_link( $term ) ); ?>"><?php echo esc_html( $term->name ); ?></a>
				</li>
			<?php endforeach; ?>
		</ul>
		<?php if ( $view_all ) : ?>
			<p class="publishing-megamenu__footer mb-0">
				<a class="publishing-megamenu__view-all" href="<?php echo esc_url( $view_all ); ?>"><?php echo esc_html( $view_label ); ?></a>
			</p>
		<?php endif; ?>
	</div>
	<?php
...
function msr_publishing_render_desktop_primary_nav() {
	...
	?>
	<nav class="publishing-desktop-nav d-none d-lg-flex" aria-label="<?php esc_attr_e( 'Primary', 'msrsandbox' ); ?>">
		<ul class="publishing-desktop-nav__list">
			<?php foreach ( $links as $index => $link ) : ?>
				...
				$panel_id   = 'publishing-megamenu-' . sanitize_html_class( $mega !== '' ? $mega : 'link-' . $index );
				$item_class = 'publishing-desktop-nav__item';
				if ( $mega !== '' ) {
					$item_class .= ' publishing-desktop-nav__item--mega';
				}
				if ( $is_current ) {
					$item_class .= ' is-current';
				}
				?>
				<li class="<?php echo esc_attr( $item_class ); ?>"<?php echo $mega !== '' ? ' data-publishing-megamenu' : ''; ?>>
					<?php if ( $mega !== '' ) : ?>
						<span class="publishing-desktop-nav__row">
							<button
								type="button"
								class="publishing-desktop-nav__trigger"
								aria-expanded="false"
							...
							<a
								class="publishing-desktop-nav__link"
